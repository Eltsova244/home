...
async function activate(context) {
    ...
    let webViewGlobal; // Global reference to a webview
    let currentLabFolderPath; // Current opened lab folder
    ...
    context.subscriptions.push(editor_1.LabEditorProvider.register(labViewHandler));
    async function labViewHandler(fileUri, forceUpdate = true) {
        ...
        if (fs.existsSync(configFilePath)) {
            ...
            if (forceUpdate && yamlConfig != undefined) {
                if ('url' in yamlConfig) {
                    ...
                    try {
                        ...
                        const res = await axios_1.default.get(`${githubRawURL}`, headers);
                        if (res.status === 200) {
                            ...
                            vscode.window.showErrorMessage(`Failed to download README.md. HTTP Status Code: ${res.status})`);
                        }
                    }
                    catch (error) {
                        console.log(error);
            ...
            const htmlCleanedUp = (0, node_html_parser_1.parse)(markdownToHtml);
            ...
            engine.parseAndRender(htmlCleanedUp.toString()).then(async (parsedHtml) => {
                const scriptUri = webViewGlobal.webview.asWebviewUri(vscode.Uri.joinPath(context.extension.extensionUri, STATIC_FOLDER, LAB_WEBVIEW_SCRIPT));
                const styleUri = webViewGlobal.webview.asWebviewUri(vscode.Uri.joinPath(context.extension.extensionUri, STATIC_FOLDER, LAB_WEBVIEW_STYLESHEET));
            ...
            await vscode.commands.executeCommand("setContext", "lab50:showReadme", false);
    ...
    function htmlTemplate(base, scriptUri, styleUri, html) {
        const fontawesomeUri = webViewGlobal.webview.asWebviewUri(vscode.Uri.joinPath(context.extension.extensionUri, `${STATIC_FOLDER}/vendor/fontawesome/css/all.min.css`));
        const mathjaxUri = webViewGlobal.webview.asWebviewUri(vscode.Uri.joinPath(context.extension.extensionUri, `${STATIC_FOLDER}/vendor/mathjax/js/tex-chtml.js`));
        const highlightjsUri = webViewGlobal.webview.asWebviewUri(vscode.Uri.joinPath(context.extension.extensionUri, `${STATIC_FOLDER}/vendor/highlightjs/js/11.6.0/highlight.min.js`));
        const highlightStyleUri = webViewGlobal.webview.asWebviewUri(vscode.Uri.joinPath(context.extension.extensionUri, `${STATIC_FOLDER}/vendor/highlightjs/css/11.6.0/styles/github-dark.min.css`));
    ...
    function validate(yamlConfig) {
        ...
        let xProvided = false;
        if ('cmd' in yamlConfig) {
            cmdProvided = true;
        }
        if ('files' in yamlConfig) {
            filesProvided = true;
        }
        if ('port' in yamlConfig) {
            portProvided = true;
        }
        if ('readme' in yamlConfig) {
            readmeProvided = false;
        }
        if ('window' in yamlConfig) {
            windowProvided = true;
            if (yamlConfig['window'].includes('browser')) {
                browserProvided = true;
            }
            if (yamlConfig['window'].includes('terminal')) {
                terminalProvided = true;
            }
            if (yamlConfig['window'].includes('x')) {
                xProvided = true;
            }
        }
        if (cmdProvided && !terminalProvided) {
            isValid = false;
            console.log(`violation: "cmd" provided but not "temrinal"`);
        }
        if (browserProvided && xProvided) {
            isValid = false;
            console.log(`violation: "browser" and "x" cannot co-exist`);
        }
        if (portProvided && !browserProvided) {
            isValid = false;
            console.log(`violation: "port" provided but not "browser"`);
        }
        return isValid;
    }
    ...
    async function initWebview() {
        if (webViewGlobal === undefined) {
            await vscode.commands.executeCommand("setContext", "lab50:showReadme", true);
            await vscode.commands.executeCommand('lab50.focus');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('lab50.openAsLab', labViewHandler));
    // Command: Reset Lab View
    context.subscriptions.push(vscode.commands.registerCommand('lab50.resetLayout', () => {
        labViewHandler({ path: currentLabFolderPath }, false);
        webViewGlobal.webview.postMessage({ command: 'reload' });
    }));
    // Command: Close Lab
    context.subscriptions.push(vscode.commands.registerCommand('lab50.closeLab', async () => {
        // Update context
        await vscode.commands.executeCommand("setContext", "lab50:showReadme", false);
